...
public class RestEndpointTest extends GeoServerTestSupport {
    ...
    public void testValidPngFreemarkerTemplateRender() throws Exception {
        ...
        MockHttpServletResponse response = postAsServletResponse("/rest/printng/freemarker/foo.png?foo=bar", "");
        assertEquals("Invalid response", Status.SUCCESS_OK.getCode(), response.getStatusCode());
    }

    private void createPrintngFreemarkerTemplate(String templateName, String templateContents) throws IOException {
        Writer writer = PrintngFreemarkerTemplateFacade.newTemplateWirter(templateName);
